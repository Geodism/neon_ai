...
def _purchase_order_document_template_seed_specs() -> list[dict[str, object]]:
    return [
        {
            "template_name": "PurchaseOrderHeader",
            "template_kind": DocumentTemplateKind.HEADER.value,
            "subject_line": None,
            "body_content": """
<header>
  <h1>{CompanyName}</h1>
  <h2>{DocumentTitle}</h2>
  <p><strong>Purchase Order #:</strong> {PurchaseOrderNumber}</p>
  <p><strong>Work Order #:</strong> {WorkOrderID}</p>
...
""".strip(),
            ...
            "change_summary": "Initial seeded purchase order header template",
            "notes": "Seeded purchase order header template for Create Purchase Order drafting workflow.",
        },
        {
            "template_name": "PurchaseOrderBody",
            "template_kind": DocumentTemplateKind.BODY.value,
            "subject_line": None,
            "body_content": """
<section>
  ...
  <p><strong>Work Order:</strong> {WorkOrderID}</p>
  <p><strong>Vendor:</strong> {VendorName}</p>
  <p><strong>Expected Arrival:</strong> {ExpectedArrivalDate}</p>
  <p><strong>Project / Site:</strong> {SiteName}</p>
  <p><strong>Ship To:</strong> {SiteAddress}</p>
  <p>Please supply the following materials:</p>
  {{PurchaseOrderLineTable}}
  <p><strong>Total:</strong> {POTotalFormatted}</p>
  <p><strong>Notes:</strong> {PurchaseOrderNotes}</p>
</section>
""".strip(),
            "token_schema": [
                "PurchaseOrderID",
            ...
            "change_summary": "Initial seeded purchase order body template",
            "notes": "Seeded purchase order body template for Create Purchase Order drafting workflow.",
        },
        {
            "template_name": "PurchaseOrderFooter",
            "template_kind": DocumentTemplateKind.FOOTER.value,
            "subject_line": None,
            "body_content": """
<footer>
...
""".strip(),
            ...
            "change_summary": "Initial seeded purchase order footer template",
            "notes": "Seeded purchase order footer template for Create Purchase Order drafting workflow.",
        },
    ]


def ensure_purchase_order_document_templates(force_defaults: bool = False) -> dict[str, object]:
    document_type_code = "PURCHASE_ORDER"
    usage_context = "PURCHASE_ORDER_DRAFT_WORKSPACE"
    specs = _purchase_order_document_template_seed_specs()
    actions: dict[str, object] = {
    ...
    try:
                ...
                if not document_type_row:
                    cur.execute(
                        ...
                        (
                            document_type_code,
                            "Purchase Order",
                            "Purchase order document templates for Create Purchase Order drafting and preview.",
                            DocumentOutputFormat.HTML.value,
                        ),
                    )
        ...
        raise RuntimeError(
            f"Purchase order template seed failed because the database connection is unavailable: {exc}"
        ) from exc
    except psycopg2.Error as exc:
        if exc.pgcode in {
            errorcodes.UNDEFINED_TABLE,
